...
export function DocumentForm({ type, onNavigate, onBack }: DocumentFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (config.needsSelection) {
      ...
      // Generate protocol and schema numbers for outgoing documents
      const protocolNum = Math.floor(Math.random() * 10000) + 1000;
      const schemaNum = Math.floor(Math.random() * 1000) + 100;
